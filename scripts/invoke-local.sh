...
then
  echo "============================================================================================================="
  echo "  Missing LAYER_REPO value inside ./script/.env file"
  echo "============================================================================================================="
  exit 1
fi
...
then
    ...
    BRANCH_BEHIND=$(git status)
    if [[ ${BRANCH_BEHIND} == *"Your branch is behind"* ]]
    then
        BRANCH_BEHIND="true"
    else
        BRANCH_BEHIND="false"
...
do
    ...
    if [[ "$CURRENT_DATE" > "$DATE_CREATION_LAYER" ]] || [ $DATE_CREATION_LAYER = "not_created" ] || [ $BRANCH_BEHIND = "true" ]
    then
        ...
        if [ "$layer" != "MiddyDependenciesNodeModule" ]
        then
            if [ "$layer" == "MiddyDependenciesPrismaOrm" ]
            then
                CURRENT_FOLDER=$(pwd)
                # echo $ROOT_FOLDER_LAYERS
                # ls -la
                DIRECTORYS=$(find . -type d | perl -lne 'print tr:/::, " $_"' | sort -n | cut -d' ' -f2)
                for dir in $DIRECTORYS
                do
                    if [ -d "$dir/.prisma" ]
                    then
                        cd $dir
                        ORIGIN_CLIENT=$(echo $(npm run project:name) | rev | cut -d' ' -f1 | rev)
                        rm -rf /opt/$ORIGIN_CLIENT
                        git clone git@github.com:$USER_GITHUB/$ORIGIN_CLIENT.git /opt/$ORIGIN_CLIENT
                        CREDENTIALS=$(sed -n '/url/p' .prisma/client/schema.prisma)
                        sed -i "/provider = \"prisma-client-js\"/a output = \"$CURRENT_FOLDER/$dir/.prisma/client\"" /opt/$ORIGIN_CLIENT/prisma/schema.prisma
                        sed -i "/binaryTargets/c\binaryTargets = [\"native\"]" /opt/$ORIGIN_CLIENT/prisma/schema.prisma
                        sed -i "/url/c\ $CREDENTIALS" /opt/$ORIGIN_CLIENT/prisma/schema.prisma
                        npm i --prefix /opt/$ORIGIN_CLIENT
                        npm i
...
echo "run serverless"
serverless invoke local --f ${FUNCTION_NAME} --p ${PATH_SERVERLESS_PARAMETER}  --aws-profile ${AWS_PROFILE}
